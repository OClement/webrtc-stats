...
export class WebRTCStats extends EventEmitter {
  constructor(options = {}) {
    ...
    if (!this.getStatsInterval || !Number.isInteger(this.getStatsInterval)) {
      throw new Error(
        `getStatsInterval should be an integer, got: ${options.getStatsInterval}`
      );
    }
  ...
  wrapGetUserMedia(options = {}) {
    ...
    let origGetUserMedia = navigator.mediaDevices.getUserMedia.bind(
  ...
  wrapGetDisplayMedia() {
    ...
    if (navigator.mediaDevices && navigator.mediaDevices.getDisplayMedia) {
      let origGetDisplayMedia = navigator.mediaDevices.getDisplayMedia.bind(
        navigator.mediaDevices
      );
      let gdm = function() {
        self.debug(
  ...
  removePeer(peerId) {
    ...
    if (!this._peersToMonitor[options.peerId]) {
      console.warn(`We are not currently monitoring peer with id ${peerId}.`);
      return;
    }

    delete this._peersToMonitor[options.peerId];
  }
            ...
            prom
              .then((res) => {
                ...
                if (this.filteredStats === true) {
                  statsEventObject["filteredStats"] = this.filteroutStats(
                    statsObject
                  );
                }
